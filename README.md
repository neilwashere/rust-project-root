...
#
create_content_blob() {
    declare content_base64
    content_base64=$(echo "$1" | base64 |  sed -z 's/\n//g')
...
modify_content() {
    ...
    current_value=$(echo "$content" | grep $key | cut -d " " -f 4 | sed "s/\"//g")
    ...
    if grep -E "$client_id(,|$)" <(echo "$current_value"); then
        ...
        declare new_content
        new_content=$(echo "$content" | sed -r "s/(^.*$key: ).*$/\1\"${new_value}\"/")
...
content=$(get_current_content "$file" "$default_branch" | sed -z 's/\n//g' | jq -rcC .content | base64 -d)
printf %s "$content"
# content=$(get_current_content "$file" "$default_branch" | jq -r .content | base64 -d)
